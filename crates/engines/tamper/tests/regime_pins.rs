...
//! resolver picked. When they drift, the parse-error delta and every AST-shaped
//! detector move at an apparently-equal `measurement_regime` — and an equal
//! regime is precisely what tells the verifier a digest disagreement is
//! tampering rather than skew (PREMORTEM S4 feeding Story 1). This test is the
//! only thing making the two equal.
//!
//! Duplicated from the clone engine's, because the two crates carry their own
//! pins and a guard that only watched one of them would leave the other exactly
//! as exposed.
//!
...
use andon_engine_tamper::syntax::{rule_pack_version, GRAMMAR_PINS};
...
fn the_pins_are_sorted_so_the_regime_string_is_stable() {
    ...
    assert_eq!(
        names, sorted,
        "rule_pack_version() joins GRAMMAR_PINS in order; an unsorted \
...
fn the_regime_string_names_the_runtime_as_well_as_the_grammars() {
    let revision = rule_pack_version();
    assert!(
