...
Gates (mirror `.github/workflows/ci.yml`): `cargo fmt --all --check` · `cargo clippy --workspace --all-targets -- -D warnings` · `RUSTDOCFLAGS="-D warnings" cargo doc --workspace --no-deps` · `cargo test --workspace --no-fail-fast` · `cargo deny check licenses bans sources`
