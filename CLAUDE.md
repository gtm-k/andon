...
Gates (mirror `.github/workflows/ci.yml` and `dogfood.yml`): `cargo fmt --all --check` · `cargo clippy --workspace --all-targets -- -D warnings` · `RUSTDOCFLAGS="-D warnings" cargo doc --workspace --no-deps` · `cargo test --workspace --no-fail-fast` · `cargo deny check licenses bans sources` · `bash scripts/tests/self-measure-ledger.test.sh`
