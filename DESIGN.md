# DESIGN.md — the canonical token, type and spacing spec for `site/`

**Why this one.** Andon's site is read by people who build agentic coding systems and
want to know, on one screen, whether a measurement can be trusted. It has to read as an
instrument — rigorous and calm — in a light theme and a dark theme that are each
designed rather than one inverted from the other. Of the developer-tool systems in
[VoltAgent/awesome-design-md](https://github.com/VoltAgent/awesome-design-md), the
Vercel analysis fits that brief most closely: a stark ink-on-near-white duet with a
deliberate gray ladder for every hairline and disabled state; a geometric sans (Geist,
served by Google Fonts) at a display ceiling of weight 600, sentence-case and
negatively tracked; a monospace face (Geist Mono) reserved for eyebrows, terminals and
code, which is the voice a measurement tool should speak in; and a semantic palette —
`error` red, `warning` amber, `link` blue — that maps onto Andon's verdict ladder
...
(repository licence: MIT) on 2026-08-27. Everything below this rule is the upstream
file, unedited; the paragraph above is Andon's. Where the spec names a proprietary
face, `site/style.css` loads the open substitute the spec's own "Note on Font
Substitutes" allows and gives every face a real system fallback stack.
