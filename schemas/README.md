...
| `payload-v1`, `registry-v1` | `EngineFamily::tests` and `MeasurementRegime::tests { engine_version, command, sandbox }` added | P7 | The user test-command occupant (PLAN P7, APPROACH: "user's own test command as the only v1 code-exec occupant") needs a family and a regime to stamp, and none of the five existing ones is honest — every one names a static-analysis engine, and a mis-stamped family seals consistently and passes the compare. Additive variants, not a migration: no existing record carries either spelling, nothing that already existed changed meaning, and no existing digest moves. The regime's `sandbox` field is where the payload carries the `no-net-isolation` disclosure. |
| `policy-v1` | `[sandbox]` section added: `enabled` (default `false`), `test_command` (default unset), `test_timeout_ms` (default `600000`), `env_allow` (default `[]`), `memory_limit_mb` (default unset) | P7 | The async lane's feature flag and the trusted-command policy of Codex #19. Everything defaults off: a file that never mentions `[sandbox]` parses to a policy that behaves exactly as before, and no engine joins any roster until an operator both enables the lane and declares a command. `policy_hash` does move — it hashes the parsed struct, which now carries the new section's defaults — and `policy_hash` is outside `ResultDigestInput`, so no per-result digest moves with it. |
